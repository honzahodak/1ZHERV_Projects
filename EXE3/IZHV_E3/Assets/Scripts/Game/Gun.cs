...
public class Gun : MonoBehaviour
{
    ...
    /// <summary>
    /// transform of the tip of the gun
    /// </summary>
    ...
    void FixedUpdate()
    {
        ...
        if (mFiring && fireEnabled)
        {
            ...
            var secondsPerBullet = 1.0f / (fireRate);
    ...
    public void ShootGun(Transform director)
    {
        ...
        if (shotgun)
        {
            ...
            float spreadAngle = shotgunSpread / (shotgunBullets-1);
            int halfBullets = shotgunBullets/2;
            for (int i = -halfBullets; i < halfBullets; i++)
            {
                Quaternion bulletRotation = Quaternion.Euler(director.rotation.eulerAngles + new Vector3( 0,0, spreadAngle * i+spreadAngle/2));
                SpawnBullet(gunTip.position, bulletRotation);
            }
        }
        else
        {
            SetFireRate(8.0f);
            SpawnBullet(gunTip.position, director.rotation);
        }
    }
