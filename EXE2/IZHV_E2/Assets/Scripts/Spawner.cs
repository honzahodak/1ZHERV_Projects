...
public class Spawner : MonoBehaviour
{
    ...
    /// <summary>
    /// Maximum frequency of spawning obstacles
    /// </summary>
    [SerializeField] private float maxSpawnFrequency;
    ...
    /// <summary>
    /// Spawnfrequency that gets reduced as you progras further
    /// </summary>
    private float reducingSpawnFrequencyMean;




    /// <summary>
    /// List of different sprites for obstacles
    /// </summary>
    [SerializeField] private Sprite[] obstacleSprites;
    /// <summary>
    /// List of sprites used for spriteMasks
    /// </summary>
    [SerializeField] private Sprite[] obstacleSpriteMasks;
    ...
    { 
        ResetSpawn(); 
        reducingSpawnFrequencyMean = spawnFrequencyMean;
    }
    ...
    void Update()
    {
        if(reducingSpawnFrequencyMean > maxSpawnFrequency)
        {
            reducingSpawnFrequencyMean = spawnFrequencyMean - GameManager.Instance.mCurrentScore/5;
        }
        //print(reducingSpawnFrequencyMean);
        if (spawnObstacles)
        { // Check if we should spawn.
            ...
            { // Spawn at most one obstacle per frame.
                ...
                nextSpawnIn = RandomNormal(reducingSpawnFrequencyMean, spawnFrequencyStd);
    ...
    public void SpawnObstacle()
    {
        ...
        float positionSlot = Random.value;
        if (positionSlot <= 0.35)
        {
            obstacle.transform.position += (Vector3)(spawnOffset); //top row
            obstacle.transform.GetChild(0).GetComponent<SpriteRenderer>().sprite = obstacleSprites[0];
        }
        else if (positionSlot <= 0.5)
        {
            obstacle.transform.position += (Vector3)(spawnOffset / 2.2f);//upper row
            obstacle.transform.GetChild(0).GetComponent<SpriteRenderer>().sprite = obstacleSprites[1];
        }
        else if (positionSlot >= 0.65)
        {
            obstacle.transform.position -= (Vector3)(spawnOffset); //bottom row
             obstacle.transform.GetChild(0).GetComponent<SpriteRenderer>().sprite = obstacleSprites[3];
        }
        else if (positionSlot > 0.5)
        {
            obstacle.transform.position -= (Vector3)(spawnOffset / 2.2f); //lower row
            obstacle.transform.GetChild(0).GetComponent<SpriteRenderer>().sprite = obstacleSprites[2];
        }
        //Change the sprite mask
        obstacle.transform.GetChild(0).GetComponent<SpriteMask>().sprite = obstacleSpriteMasks[(int)Math.Floor(Random.Range(0.0f, 3.1f))];
